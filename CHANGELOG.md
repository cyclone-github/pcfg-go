### v0.6.1-dev.20260907-1815; 2026-09-07
- `pcfg_guesser -adaptive` mode creates a live feedback loop which steers PCFG runtime probabilities towards those which are generating founds
- Smaller default stdout/PT queues (256/8/64K); `-adaptive` uses a shallower pipe (2/4/32K) to cut founds lag
- Fixed Ctrl+C shutdown with `-adaptive` mode
- Fixed `-adaptive` prior normalizations
- Added trainer and guesser CLI flag sanity checks
